...
`教职工出口` 对应后缀为 `@jzg`，登录时会提交为 `DDDDD=账号@jzg`。该出口有接入环境限制，必须在学校网关允许的对应 IP / 接入位置下使用；如果当前电脑获取到的内网 IP、MAC、NAS 或端口信息与该出口不匹配，即使账号和密码正确，也可能认证失败。
...
出现该错误时，通常不是密码错误，也不是 NetKeeper 拼接参数错误，而是当前接入环境不满足教职工出口的认证条件。处理建议：

- 确认当前网络环境是否属于教职工出口允许的 IP / 接入区域。
- 如果当前处于学生网络环境，请选择学生电信、学生联通或学生移动出口。
- 如果确认应当使用教职工出口但仍报错，需要检查学校网关侧账号、MAC、IP、NAS 或端口绑定情况。
